...
import verfyCodeAction from "@/lib/actions/restPasswordActions/verfyCode.action";
import {
  verfyCode,
  verfyCodeType,
} from "@/schema/(rest Password)/verfyCode.schema";
...
export default function VerifyResetCode() {
  ...
  //----------- start use form------------
  const form = useForm<verfyCodeType>({
    defaultValues: {
      resetCode: "",
    },
    mode: "onChange",
    resolver: zodResolver(verfyCode),
  });
  //----------- end use form------------

// ---------------------start fn call api ------------------
async function handleVerfyCode(values: verfyCodeType) {
  //   console.log(values);
  setisLoading(true)
  
  try {
    const res = await verfyCodeAction(values);
    if (res.status === "Success") {
      // console.log(res);
      toast.success(`Verfy Code Successfuly ya bro🖤`, {
        duration: 3000,
      });
    ...
    toast.error(err instanceof Error ? err.message : "can't verfy code!!");
    setisLoading(false);
  }
}
// ---------------------end fn call api ------------------
        ...
        <StyledWrapper className="">
          <div className="card flex items-center w-[320px]  justify-center mx-auto md:w-[410px] xl:w-[510x] md:h-[280] ">
            ...
            <div className="content">
              ...
              <p className="sub-heading">verfy code</p>

              <Form {...form}>
                <form onSubmit={form.handleSubmit(handleVerfyCode)}>
                  <p className="sub-sub-heading text-center">Type verfy code</p>
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <input
                            {...field}
                            className="resetCode w-full mt-0 pt-0"
